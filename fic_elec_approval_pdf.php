...
$sql2= "SELECT * FROM `stock_issue_recurring_electrical` ORDER BY sno DESC LIMIT 1";
        $result1 = mysqli_query($con, $sql2);
        $row = mysqli_fetch_assoc($result1);
        $last_si_no= $row["sno"];

$sql = "SELECT * FROM `stock_issue_recurring_electrical` WHERE `sno` = '$last_si_no'";
...
if(mysqli_num_rows($res)>0){
  ...
  $html.="<h2>FIC Electrical - Approval</h2>";
  $html.= '<hr>';
...
$mpdf=new Mpdf\Mpdf();
$mpdf->WriteHTML($html);
$file='Approval_Electrical/'.time().'.pdf';
...
$sql="INSERT INTO `fic_approval_electrical`(`pdf_path`) 
VALUES ('$file')";
...
if($result){ 
  $insert= true;
  header('Location: edit_SR_Electrical.php');
}
else{
